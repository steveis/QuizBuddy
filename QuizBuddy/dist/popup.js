...
import { loginWithGoogle, createQuiz, getQuizQuestions, startQuizAttempt, completeQuizAttempt } from './apiService';
// DOM Elements
...
async function handleCreateQuizFromCurrentPage() {
    try {
...
async function handleHighlightPdfs() {
    try {
        // Get current tab
...
async function createQuizFromContent(content) {
    try {
...
// Show quiz results
function showQuizResults(quizAttempt) {
    // Clear container
    quizTakingContainer.innerHTML = '';
...
async function checkPendingQuizContent() {
    try {
        const data = await chrome.storage.local.get(['pendingQuizContent']);
        if (data.pendingQuizContent) {
...
// Scan for PDF links on the current page
async function scanForPdfLinks() {
    try {
        // Get current tab
        const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
        const currentTab = tabs[0];
...
// Show quiz creation state
function showQuizCreationState(message) {
...
function hideQuizCreationState() {
